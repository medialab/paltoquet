...
type Step = Vec<(i32, &'static str, Option<&'static str>)>;
...
lazy_static!{
    ...
    static ref M: Regex = {
        Regex::new(&format!("(?i)([{}]+[^{}]+)", VOWELS, VOWELS)).unwrap()
    };

    static ref STEP1: Step = vec![
        (0, "issaient", None),
        (0, "ellement", Some("el")),
        (0, "issement", None),
        (0, "alement", Some("al")),
...
pub fn apply_rules(rules: Step, stem: &str) -> String{
    for i in 0..rules.len() {
        let min: i32 = rules[i].0;
...
pub fn carry_stemmer(word: &str) -> String {
    ...
    mut_word = apply_rules(STEP1.clone(), &mut_word);
    mut_word = apply_rules(STEP2.clone(), &mut_word);
    mut_word = apply_rules(STEP3.clone(), &mut_word);
...
mod tests {
    ...
    #[test]
    fn test_apply_rules() {
        assert_eq!(apply_rules(STEP1.clone(), "Tissaient"), "Tiss");
    }
